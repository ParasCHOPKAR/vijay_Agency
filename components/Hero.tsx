...
export default function Hero() {
  ...
  // State to track if the house lights are on or off
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 1. TEXT REVEAL: Crisp stagger up for left side content
      tl.from(".gsap-reveal", {
        y: 40,
        opacity: 0,
        duration: 1,
      ...
      // 2. HOUSE ENTRANCE: Slides in from right to left
      .from(".gsap-house", {
        x: 150, // Starts 150px to the right and slides left into position
        opacity: 0,
        duration: 1.5,
        ease: "power3.out",
      }, "-=0.6") 
      
      // 3. LABELS POP IN: Springy pop effect AFTER the house arrives
      .from(".gsap-label", {
        scale: 0.5,
        opacity: 0,
        duration: 0.8,
  ...
  // Toggle handler for the house image
  const toggleLight = () => {
    ...
    <section ref={sectionRef} className={styles.hero}>
      <div className={styles.container}>
        ...
        <div className={styles.visualsWrapper}>
          {/* TIGHT WRAPPER: Ensures labels stay exactly on top of the image coordinates */}
          <div className={styles.imageInnerWrapper}>
            {/* The isolated house image */}
            <img 
              src={isLightOn ? "/hero-house-011111.png" : "/hero-house-light-off_01.png"} 
            ...
            {/* Labels positioned directly over the image to match the screenshot */}
